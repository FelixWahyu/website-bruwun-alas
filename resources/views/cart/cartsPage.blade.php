            ...
            @if ($carts->isEmpty())
                <div class="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-100">
                    ...
                    <a href="{{ route('katalogProduk') }}"
                        class="inline-flex items-center justify-center px-8 py-3 bg-red-600 text-white font-bold rounded-full hover:bg-red-700 transition shadow-lg hover:-translate-y-1">
                        Mulai Belanja
                    </a>
                </div>
            @else
                <div class="flex flex-col lg:flex-row gap-8 items-start">
                    <div class="lg:w-2/3 space-y-4">
                        ...
                        @foreach ($carts as $cart)
                            ...
                            <div
                                class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 flex gap-4 sm:gap-6 items-start group hover:border-red-200 transition">
                                <div
                                    class="shrink-0 w-24 h-24 sm:w-28 sm:h-28 bg-gray-100 rounded-xl overflow-hidden border border-gray-100">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <h3
                                                class="text-lg font-bold text-gray-900 line-clamp-1 hover:text-red-600 transition">
                                        ...
                                        <form action="{{ route('cart.destroy', $cart->id) }}" method="POST">
                                            ...
                                            <button type="submit" onclick="confirmDeleteCart(event)"
                                                class="text-gray-400 hover:text-red-500 p-2 cursor-pointer rounded-full hover:bg-red-50 transition"
                                                title="Hapus Item">
                                            ...
                                            <div class="flex items-center justify-center">
                                                @if ($cart->quantity > 1)
                                                    <form action="{{ route('cart.update', $cart->id) }}" method="POST">
                                                        ...
                                                        <button type="submit"
                                                            class="w-8 h-8 rounded-l cursor-pointer border border-gray-300 bg-gray-50 text-gray-600 hover:bg-gray-100 flex items-center justify-center font-bold transition">
                                                            -
                                                        </button>
                                                    </form>
                                                @else
                                                    <form action="{{ route('cart.destroy', $cart->id) }}" method="POST">
                                                        ...
                                                        @method('DELETE')
                                                        <button type="submit" onclick="confirmDeleteCart(event)"
                                                            class="w-8 h-8 rounded-l cursor-pointer border border-gray-300 bg-red-50 text-red-600 hover:bg-red-100 flex items-center justify-center font-bold transition">
                                                            -
                                                        </button>
                                                    </form>
                                                @endif
                                                ...
                                                <form action="{{ route('cart.update', $cart->id) }}" method="POST">
                                                    ...
                                                    <button type="submit"
                                                        class="w-8 h-8 rounded-r cursor-pointer border border-gray-300 bg-gray-50 text-gray-600 hover:bg-gray-100 flex items-center justify-center font-bold transition">
                                                        +
                                                    </button>
                                                </form>
                                            </div>
                    ...
                    <div class="lg:w-1/3 lg:sticky lg:top-24">
                        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            ...
                            <div class="flex justify-between items-center border-t border-dashed border-gray-200 pt-4 mb-6">
                                ...
                                <span class="text-2xl font-extrabold text-red-600">Rp
                                    {{ number_format($grandTotal, 0, ',', '.') }}</span>
                            </div>

                            <a href="{{ route('checkout') }}"
                                class="w-full block text-center py-4 bg-red-600 text-white font-bold rounded-xl shadow-lg hover:bg-red-700 hover:shadow-red-500/30 transition-all transform hover:-translate-y-0.5">
                                Checkout Sekarang
                            </a>
