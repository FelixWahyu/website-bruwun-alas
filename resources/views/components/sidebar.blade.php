<aside :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
    ...
    <nav class="mt-6 px-4 space-y-2">
        ...
        <a href="{{ route('admin.dashboard') }}"
            ...
            <svg class="w-5 h-5 mr-3 text-gray-400 group-hover:text-red-600 {{ request()->routeIs('admin.dashboard') ? 'text-red-600' : '' }}"
        ...
        <a href="{{ route('admin.reports.index') }}"
            class="flex items-center text-sm px-4 py-3 text-gray-600 transition-colors rounded-xl hover:bg-red-50 hover:text-red-700 group {{ request()->routeIs('admin.reports.index') ? 'bg-red-50 text-red-600 font-semibold' : '' }}">
            <svg class="w-5 h-5 mr-3 text-gray-400 group-hover:text-red-600 {{ request()->routeIs('admin.reports.index') ? 'text-red-600' : '' }}"
                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @if (auth()->user()->role === 'owner')
            <a href="{{ route('owner.charts.index') }}"
                class="flex items-center text-sm px-4 py-3 text-gray-600 transition-colors rounded-xl hover:bg-red-50 hover:text-red-700 group {{ request()->routeIs('owner.charts.index') ? 'bg-red-50 text-red-600 font-semibold' : '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                    stroke="currentColor"
                    class="w-5 h-5 mr-3 text-gray-400 group-hover:text-red-600 {{ request()->routeIs('owner.charts.index') ? 'text-red-600' : '' }}">
